...
std::unique_ptr<Listener> TcpProber::listen(int argc, char *argv[]) {
	if (argc != 2) {
	...
	if ((fd = resolve_socket_bind(argv[0], argv[1], SOCK_STREAM, IPPROTO_TCP)) == -1)
		return NULL;
...
std::unique_ptr<ConnectionProbe> TcpProber::connect(int argc, char *argv[]) {
	if (argc != 2) {
		std::cerr << name() << " listen <ip> <port>" << std::endl;
	...
	if ((fd = resolve_socket_connect(argv[0], argv[1], SOCK_STREAM, IPPROTO_TCP)) == -1)
		return NULL;
