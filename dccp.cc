...
std::unique_ptr<Listener> DccpProber::listen(int argc, char *argv[]) {
	...
	if ((fd = resolve_socket_bind(argv[0], argv[1], SOCK_DCCP, IPPROTO_DCCP)) == -1)
		return NULL;
...
std::unique_ptr<ConnectionProbe> DccpProber::connect(int argc, char *argv[]) {
	if (argc != 2) {
		std::cerr << name() << " listen <ip> <port>" << std::endl;
	...
	if ((fd = resolve_socket_connect(argv[0], argv[1], SOCK_DCCP, IPPROTO_DCCP)) == -1)
		return NULL;
